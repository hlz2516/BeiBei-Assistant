...
其实在老版本中我就发现了这个问题，只是当时没在意。点击开始背题后服务端返回的题目中，熟悉的题目和理解的题目，上一次背和这一次背，总是那么几道。
原因在于在后台，每次执行select的语句是一样的，无非是返回条数的差别。为了解决这个问题，我们在quiz表里新增一个字段叫remCount，
每次在用户选择对题目的熟练度后，将该题目的remCount进行++，每次select要背题的题目时，通过order by remCount升序排序，那么就
能解决这个问题。(其实就是上次新增功能里的第五点)  
(完成 2022/09/26)
...
1. 为了能让用户更方便地看到公共题库，在题库管理模块划分为个人题库和公共题库。其中个人题库的导入题库功能放到公共题库中，公共题库同样显示列表，新增一个下载次数列，操作列里的功能有“下载”和“评论”。公共题库板块需要添加一个搜索框，方便搜索题库；还需要添加一个筛选器，可以根据下载次数，题库创建时间进行顺序排列。公共题库需要做下分页。(完成 2022/09/23)
2. 链接在背题界面中的表现形式，将纯字符串改为a标签的形式(完成 2022/09/26)
3. 出题界面的搜索，可增加特殊搜索：<题库名> (优先)(完成 2022/09/25)
4. 背题配置项，将目前的背题模式称为“随机模式”，新增“目标模式”。在选定题库，标签，重要性，题目数量后，选择“随机模式”或“目标模式”，若选择随机模式，则点击“开始背题”后直接开始；若选择“目标模式”，在新出现一个选择题目的配置项，配置项里的所有题目均是符合前面用户选择的题目要求的，在选定指定的题目数量后，才能再“开始背题”。 (优先)(完成 2022/09/25)
5. 随机模式的选题sql逻辑中，“已熟悉”的题目随机选取返回，“已理解”的题目需按照时间顺序排列返回
6. 登录token过期不会自动跳转到登录页面，因为没有对invalid token这种情况做处理(完成 2022/09/25)
7. 给用户添加一个等级属性，每背一题增加一点经验，不同的等级有不同的经验范围

一些想法:
用户背的题库有可能是自己的，也有可能是从公共题库拷贝过来的。那么我们就要分情况考虑这两种情况，用户会分别怎么背。
1. 针对用户自己写自己背的情况。我现在就是这种情况，通常这种情况下用户都有自己的一套面试题资源，因此在题目还没有全部输入到网站前，大部分时间我是在跟着那套资源走，只是把自己总结的面试题描述记入到自己的题库中，小部分时间会使用背题功能复习下。
2. 针对用户使用公共题库的情况。这种情况一般用户会先浏览一遍该题库下所有的题目，所以需要<题目名>来搜索某个题库下所有的题目；另外，考虑到使用的用户已经背过一些面试题，他只需要针对某些不会的题目加强一下即可，所以背题配置项中增加“目标模式”，让用户可以指定题目进行背题。
