...
create procedure `interview_stage`.`deal_tags`(in tags json,in quiz_id bigint)
    BEGIN
        ...
        flag:loop
            ...
            insert into tagquizs(`tagId`,`quizId`) values(tag_id,quiz_id);
...
create procedure `interview_stage`.`download`(in player_id int,in code char(6))
    BEGIN
